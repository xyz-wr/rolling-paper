...
public class JwtTokenProvider implements InitializingBean {
    ...
    public JwtTokenProvider(PrincipalDetailsService principalDetailsService,
                            ...
                            @Value("${jwt.access-token-seconds}") Long accessTokenValidityInMilliseconds,
                            @Value("${jwt.refresh-token-seconds}") Long refreshTokenValidityInMilliseconds) {
    ...
    public boolean validateRefreshToken(String refreshToken) {
        try {
            if (redisService.getValues(refreshToken) != null && redisService.getValues(refreshToken).equals("delete")) { //회원 탈퇴했을 경우
                return false;
            }
            Jwts.parserBuilder()
                    .setSigningKey(signingKey)
