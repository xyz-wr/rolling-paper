...
public class PaperApiController {
    ...
    public ResponseEntity<List<PaperDto>> getPublicPapers(@PageableDefault(page = 0, size = 6, sort = "createdDate", direction = Sort.Direction.DESC) Pageable pageable) {
        List<PaperDto> responseDtos = paperService.getPublicPapers(pageable)
                .stream()
                .map(PaperDto::new)
                .toList();
    ...
    public ResponseEntity<List<PaperDto>> getFriendPapers(@PageableDefault(page = 0, size = 6, sort = "createdDate", direction = Sort.Direction.DESC) Pageable pageable) {
        List<PaperDto> responseDtos = paperService.getFriendPapers(pageable)
                .stream()
                .map(PaperDto::new)
                .toList();
    ...
    public ResponseEntity<List<PaperDto>> searchPublicPapers(@RequestParam("keyword") String keyword,
        ...
        List<PaperDto> responseDtos = paperService.searchPublicPapers(keyword, pageable)
                .stream()
                .map(PaperDto::new)
                .toList();
    ...
    public ResponseEntity<List<PaperDto>> searchFriendPapers(@RequestParam("keyword") String keyword,
                                                             @PageableDefault(page = 0, size = 6, sort = "createdDate", direction = Sort.Direction.DESC) Pageable pageable) {
        List<PaperDto> responseDtos = paperService.searchFriendPapers(keyword, pageable)
                .stream()
                .map(PaperDto::new)
                .toList();
